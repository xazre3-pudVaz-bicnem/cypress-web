...
const ICON_SVG: Record<string, React.ReactNode> = {
  ...
  Heading: <path strokeLinecap="round" strokeLinejoin="round" d="M2.243 4.493v7.5m0 0v7.502m0-7.501h10.5m0-7.5v7.5m0 0v7.501m4.501-8.627 2.25 2.25m0 0-2.25 2.25m2.25-2.25h-4.5" />,
...
function Icon({ name, className = "w-5 h-5" }: { name: string; className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className={className}>
      {ICON_SVG[name] ?? <circle cx="12" cy="12" r="8" />}
    </svg>
  );
}
...
export default function SubCategoryTemplate({ data }: { data: SubCategoryData }) {
  ...
  return (
    <main style={{ background: "#FFFFFF", minHeight: "100vh" }}>
      ...
      <section className="relative pt-32 pb-20 overflow-hidden" style={{ background: "#FFFFFF" }}>
        ...
        {/* Soft radial accent */}
        <div
          className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[480px] pointer-events-none"
          style={{ background: `radial-gradient(ellipse, ${data.accentColor}0d 0%, transparent 68%)` }}
        />

        <div className="relative z-10 max-w-5xl mx-auto px-6">
          {/* Breadcrumb */}
          <nav className="flex items-center gap-2 text-[12px] mb-8 flex-wrap" style={{ color: "#64748B" }}>
            {data.breadcrumb.map((b, i) => (
              <span key={b.href} className="flex items-center gap-2">
                {i > 0 && <span style={{ color: "#CBD5E1" }}>/</span>}
                {i < data.breadcrumb.length - 1 ? (
                  <Link href={b.href} className="hover:text-[#2563EB] transition-colors">
                    {b.label}
                  </Link>
                ) : (
          ...
          {/* Category badge */}
          <div className="mb-5">
            <SectionLabel color={data.accentColor}>{data.categoryLabel}</SectionLabel>
          </div>
          ...
          <motion.h1
            ...
            style={{ fontSize: "clamp(28px, 4.5vw, 56px)", color: "#0F172A" }}
          >
            {data.subtitle}
          </motion.h1>
          ...
          <motion.p
            ...
            className="text-base leading-relaxed max-w-3xl mb-10"
            style={{ color: "#334155" }}
          >
            {data.overview}
          </motion.p>
        ...
        <div className="max-w-5xl mx-auto px-6">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {data.keyPoints.map((kp, i) => (
              <ScrollReveal key={kp.title} delay={i * 0.08}>
                <div
                ...
                >
                  ...
                  >
                    <span style={{ color: data.accentColor }}>
                      <Icon name={kp.icon} className="w-5 h-5" />
                    </span>
                  </div>
                  <h3 className="font-bold text-sm mb-2" style={{ color: "#0F172A" }}>{kp.title}</h3>
                  <p className="text-xs leading-relaxed" style={{ color: "#64748B" }}>{kp.body}</p>
                </div>
              </ScrollReveal>
            ))}
          </div>
          ...
          {data.sections.map((sec, i) => (
            <ScrollReveal key={i}>
              ...
              >
                ...
                {sec.body && (
                  <p className="leading-relaxed mb-5" style={{ color: "#334155" }}>
                    {sec.body}
                  </p>
                )}
                {sec.list && (
      ...
      {data.benefits.length > 0 && (
        <section className="py-16" style={{ background: "#FFFFFF" }}>
          <div className="max-w-5xl mx-auto px-6">
            <ScrollReveal>
              <SectionLabel color={data.accentColor}>Results</SectionLabel>
              <h2 className="font-black text-2xl mb-10" style={{ color: "#0F172A" }}>
                数字で見る成果
              </h2>
            </ScrollReveal>
                ...
                <ScrollReveal key={b.label} delay={i * 0.08}>
                  ...
                  >
                    <div
                      ...
                      style={{ fontSize: "clamp(24px, 3.5vw, 40px)", color: data.accentColor }}
                    >
                      <AnimatedNumber
                    ...
                    <div className="text-sm font-semibold mb-1" style={{ color: "#0F172A" }}>{b.label}</div>
                    <div className="text-xs" style={{ color: "#64748B" }}>{b.description}</div>
                  </div>
                </ScrollReveal>
              ))}
            </div>
              ...
              {data.faq.map((item, i) => (
                <ScrollReveal key={i} delay={i * 0.05}>
                  <div className="px-6 py-5">
                    ...
                    >
                      <p className="text-sm leading-relaxed pt-3 pl-6" style={{ color: "#64748B" }}>
                        {item.a}
                      </p>
                    </div>
                  </div>
      ...
      {/* ── Related Pages ── */}
      <section className="py-16" style={{ background: "#FFFFFF", borderTop: "1px solid #F1F5F9" }}>
        <div className="max-w-5xl mx-auto px-6">
          <ScrollReveal>
            <h2 className="font-bold text-lg mb-8" style={{ color: "#0F172A" }}>
              関連ページ
            </h2>
          </ScrollReveal>
          <div className="grid sm:grid-cols-3 gap-4">
            {data.relatedPages.map((rp, i) => (
              <ScrollReveal key={rp.href} delay={i * 0.07}>
                <Link href={rp.href} className="block group">
                  <div
                    className="p-5 rounded-2xl transition-all duration-300"
                    style={{
                      background: "#FFFFFF",
                  ...
                  >
                    <div
                      className="font-semibold text-sm mb-1.5 transition-colors group-hover:text-[#2563EB]"
                      style={{ color: "#0F172A" }}
                    >
                      {rp.title}
                    </div>
                    <div className="text-xs" style={{ color: "#64748B" }}>{rp.description}</div>
                    <div
                      className="mt-3 text-xs font-medium flex items-center gap-1 transition-colors group-hover:text-[#2563EB]"
                      style={{ color: "#94A3B8" }}
                    >
                      詳細を見る
      ...
      <section className="py-24" style={{ background: "#2563EB" }}>
        <div className="max-w-4xl mx-auto px-6 text-center">
          <ScrollReveal>
            <h2 className="font-black text-white text-2xl md:text-3xl mb-4 leading-tight">
              お気軽にご相談ください
            </h2>
            <p className="mb-8" style={{ color: "rgba(255,255,255,0.8)" }}>
              現状の課題をヒアリングし、最適な戦略をご提案します。
            </p>
            <Link
              href="/contact"
