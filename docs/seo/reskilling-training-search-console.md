...
| URL | 役割 | メインキーワード |
| --- | --- | --- |
| `/training` | ハブページ（第2弾で追加。それまで404だった） | 法人研修 / 企業研修 相談 |
...
### 専門コラム 第2弾（30本｜業種別・役職別・ツール別・比較・費用・制度コース・地域）

| URL | メインキーワード |
| --- | --- |
| `/column/restaurant-dx-training` | 飲食店 DX研修 |
| `/column/construction-dx-training` | 建設業 DX研修 |
| `/column/clinic-dx-training` | クリニック DX研修 |
| `/column/beauty-salon-ai-training` | 美容室 AI研修 |
| `/column/manufacturing-dx-training` | 製造業 DX研修 |
| `/column/retail-dx-training` | 小売業 DX研修 |
| `/column/realestate-ai-training` | 不動産 AI研修 |
| `/column/logistics-dx-training` | 物流 DX研修 |
| `/column/professional-office-ai-training` | 士業 AI研修 |
| `/column/cram-school-ai-training` | 学習塾 AI研修 |
| `/column/manager-ai-training` | 管理職向け AI研修 |
| `/column/newcomer-ai-training` | 新入社員 AI研修 |
| `/column/executive-dx-training` | 経営者 DX研修 |
| `/column/copilot-training-for-companies` | Copilot 研修 法人 |
| `/column/gemini-training-for-companies` | Gemini 研修 法人 |
| `/column/reskilling-vs-recurrent` | リスキリング リカレント 違い |
| `/column/subsidy-vs-grant-training` | 助成金 補助金 違い |
| `/column/group-vs-online-training` | 集合研修 オンライン研修 比較 |
| `/column/inhouse-vs-outsource-training` | 社員研修 内製 外注 |
| `/column/elearning-vs-workshop-training` | eラーニング 集合研修 比較 |
| `/column/ai-training-cost-guide` | AI研修 費用 |
| `/column/employee-training-cost-guide` | 社員研修 費用 |
| `/column/training-plan-guide` | 研修計画 立て方 |
| `/column/training-effect-measurement` | 研修 効果測定 |
| `/column/training-retention-guide` | 研修 定着 |
| `/column/jinzai-ikusei-shien-course` | 人材育成支援コース |
| `/column/hito-heno-toushi-course` | 人への投資促進コース |
| `/column/kyoiku-kunren-kyuka-course` | 教育訓練休暇等付与コース |
| `/column/tokyo-corporate-training` | 東京 法人研修 |
| `/column/katsushika-corporate-training` | 葛飾区 研修 |

※制度コース記事（`jinzai-ikusei-shien-course` / `hito-heno-toushi-course` / `kyoiku-kunren-kyuka-course`）は、コース名で検索する層を受け止めるページ。細かい支給要件・助成率・訓練時間数は**断定せず**、「どこに確認しに行くべきか」を示す構成にしている。制度改定時は真っ先に見直すこと。
...
**最優先（初日）**
1. `/training`（ハブ）
2. `/training/reskilling-subsidy`
3. `/training/hr-development-subsidy-guide`
4. `/training/ai-dx-reskilling`
5. `/training/chatgpt-ai-training`
6. `/training/web-marketing-training`

**次点（2日目）**
7. `/training/reskilling-training-company-referral`
8. `/partner/training-company-referral`
9. `/column/category/training`
10. `/column/hr-development-subsidy-reskilling`
11. `/column/reskilling-training-subsidy`

**3日目以降**
残りのコラム43本を、キーワードの検索ボリュームが大きいと想定される順に登録リクエストする。
目安の優先順位は次のとおり。

1. 助成金系（`ai-training-subsidy` / `dx-training-subsidy` / `web-marketing-training-subsidy` / `subsidy-training-company-selection`）
2. 制度コース系（`jinzai-ikusei-shien-course` / `hito-heno-toushi-course` / `kyoiku-kunren-kyuka-course`）— コース名は指名検索に近く、競合が薄い
3. 役職・ツール系（`manager-ai-training` / `copilot-training-for-companies` / `gemini-training-for-companies`）
4. 業種別（`restaurant-dx-training` ほか10本）
5. 比較・費用・実務（`reskilling-vs-recurrent` / `ai-training-cost-guide` ほか）
6. 地域（`tokyo-corporate-training` / `katsushika-corporate-training`）
...
### 業種別（第2弾コラムで狙う）
- 飲食店 DX研修 / 建設業 DX研修 / 製造業 DX研修 / 小売業 DX研修 / 物流 DX研修
- クリニック DX研修 / 美容室 AI研修 / 不動産 AI研修 / 士業 AI研修 / 学習塾 AI研修

### 役職別・ツール別（第2弾コラムで狙う）
- 管理職向け AI研修 / 新入社員 AI研修 / 経営者 DX研修
- Copilot 研修 法人 / Gemini 研修 法人

### 比較・費用・実務（第2弾コラムで狙う）
- リスキリング リカレント 違い / 助成金 補助金 違い
- 集合研修 オンライン研修 比較 / eラーニング 集合研修 比較 / 社員研修 内製 外注
- AI研修 費用 / 社員研修 費用 / 研修計画 立て方 / 研修 効果測定 / 研修 定着

### 制度コース名（指名検索に近く競合が薄い）
- 人材育成支援コース / 人への投資促進コース / 教育訓練休暇等付与コース
...
| 指標 | 見方 | 目安 |
| --- | --- | --- |
| インデックス済みページ数 | 「ページ」レポート | 追加54ページ（研修8＋紹介モデル1＋コラム45）がすべて「登録済み」になっているか |
