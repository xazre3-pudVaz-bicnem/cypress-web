...
const AREA = "katsushika";
const AREA_LABEL = "葛飾区";
const AREA_HREF = "/area/katsushika";
const BASE = "https://www.cypress-all.co.jp";

// 専用ページ（静的ルート）を持つslugは除外する
const STATIC_ROUTE_SLUGS = ["web-design"];

export async function generateStaticParams() {
  return getAreaSubPagesByArea(AREA)
    .filter((p) => !STATIC_ROUTE_SLUGS.includes(p.slug))
    .map((p) => ({ slug: p.slug }));
}
    ...
    <>
      ...
      <main>
        ...
        <section style={{ background: "#f8f6f2", padding: "64px 0" }}>
          <div style={{ maxWidth: "72rem", margin: "0 auto", padding: "0 24px" }}>
            <p style={{ fontFamily: "var(--font-display)", letterSpacing: "0.25em", color: "#9ca3af", fontSize: "11px", marginBottom: "12px" }}>Katsushika Services</p>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "10px" }}>
              {[
                ...
                { label: "葛飾区のHP制作・補助金活用", href: "/area/katsushika/web-design-subsidy" },
                { label: "葛飾区の集客支援トップ", href: "/area/katsushika" },
              ].map((l) => (
        ...
        <PageContactCTA
          ...
          body={`${AREA_LABEL}拠点のサイプレスが、地域特性を踏まえた集客支援を提供します。訪問相談・お見積もりは無料です。お気軽にご相談ください。`}
        />
      </main>
      <Footer />
    </>
