  ...
  "@graph": [
    ...
    },
    {
      ...
      mainEntity: [
        ...
        { "@type": "Question", name: "Webサイトの表示速度はSEOに影響しますか？", acceptedAnswer: { "@type": "Answer", text: "はい、表示速度（Core Web Vitals）はGoogleのSEO評価要素の一つです。LCP（最大コンテンツの描画時間）2.5秒以内、INP（インタラクション応答）200ms以下、CLS（レイアウトのずれ）0.1以下がGoogleの推奨基準です（2024年3月よりFIDはINPに置き換わりました）。Next.jsによる制作・画像最適化・フォントの最適化・不要なJavaScriptの削減などで改善できます。" } },
...
const faqItems = [
  ...
  },
  {
    ...
    a: "はい、表示速度（Core Web Vitals）はGoogleのSEO評価要素の一つです。LCP（最大コンテンツの描画時間）2.5秒以内、INP（インタラクション応答）200ms以下、CLS（レイアウトのずれ）0.1以下がGoogleの推奨基準です（2024年3月よりFIDはINPに置き換わりました）。Next.jsによる制作・画像最適化・フォントの最適化・不要なJavaScriptの削減などで改善できます。",
  },
  {
